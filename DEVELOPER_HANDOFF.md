...
1. **MacroList Purge:** Invoke `MacroListProvider.deleteAllListsOfType(typeId)`. This will hunt down every list attached to the dying Type and physically wipe its underlying items from local storage.
2. **Settings Purge:** Invoke `SettingsProvider.deleteCustomType(typeId)` to wipe the taxonomy metadata, grouping dictionaries, and anchor logic.
...
UI bottom sheets (`FluidEditSheet` vs `BatchActionBar`) are mathematically isolated to prevent render freezing and collision.
...
* These variables must be mutually exclusive. If a user begins dragging, or selects a batch item, the `editItemId` must be nulled out instantly to prevent "Zombie" item tap locks.